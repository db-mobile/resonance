...
export class EnvironmentService {
    ...
    async exportEnvironment(environmentId) {
        try {
            ...
            // Secret values live in the SecretStore and are intentionally NOT exported;
            // only the secretKeys flag travels so an import re-marks them as secret.
            return {
        ...
        try {
            ...
            const environmentsData = {
                items: data.environments.map(env => ({
                    ...
                    variables: env.variables || {},
                    // Preserve the secret flag; imported secret values are empty until set
                    secretKeys: Array.isArray(env.secretKeys) ? env.secretKeys : [],
