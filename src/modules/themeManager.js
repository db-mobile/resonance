export class ThemeManager {
    ...
    async loadSavedTheme() {
        try {
            let savedTheme = await window.electronAPI.store.get('theme');
    ...
    async loadSavedAccent() {
        try {
            const savedAccent = await window.electronAPI.store.get('accent');
            this.currentAccent = savedAccent || 'green';
        } catch (error) {
            console.error('Error loading saved accent:', error);
            this.currentAccent = 'green';
        }
    }
    ...
    async saveAccent(accent) {
        try {
            await window.electronAPI.store.set('accent', accent);
        } catch (error) {
    ...
    async setAccent(accent) {
        if (!this.availableAccents.includes(accent)) {
            console.error(`Accent '${accent}' is not available`);
            return;
        }
